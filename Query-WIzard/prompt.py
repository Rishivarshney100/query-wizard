...
- **Q:** "Tell me the roll number,name and class of those students whose roll number is prime, result marks are greater than the overall average marks and name contains the letter 't' considering roll number as integer.
- **A:** `SELECT s.roll_number, s.name, s.class
FROM student s
JOIN result r ON s.roll_number = r.roll_number
WHERE s.roll_number IN (
    -- Generate prime numbers (basic way up to a certain limit)
    SELECT n
    FROM generate_series(2, 1000) AS n  -- adjust range as per your roll number limit
    WHERE NOT EXISTS (
        SELECT 1
        FROM generate_series(2, floor(sqrt(n))) AS x
        WHERE n % x = 0
    )
)
AND r.marks > (
